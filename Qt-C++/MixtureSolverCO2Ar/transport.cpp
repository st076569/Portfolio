...
void TemperatureNDc::calcT12(double e12)
{
    // Вспомогательные переменные
    int nL = 0;
...
void TemperatureNDc::calcT3(double e3)
{
    // Вспомогательные переменные
    int nL = 0;
    int nR = vE3_v.size() - 1;
...
void TemperatureNDc::calcTv(double ev)
{
    // Вспомогательные переменные
    int nL = 0;
    int nR = vE_v.size() - 1;
...
const double &FlowMembers::tDiffQ() const
{
    return tDiffQ_s;
}
...
void FlowMembersDc::computeTDiffQ(const MacroParam& param)
{
    tDiffQ_s = 0.0;
    for (int i = 0; i < N_SORTS; ++i)
    {
        tDiffQ_s -= param.p * d_v[i] * transport_.tDiffusion()[i];
    }
}
